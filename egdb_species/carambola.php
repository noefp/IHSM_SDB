<br>
<img class="float-right" style="z-index:0;height:150px" src="<?php echo $images_path.'/species/'.$sps_img ?>" >
<h1><?php echo $sps_subtitle ?></h1>
<h3 style="color:#666"><em><?php echo $sps_title ?></em></h3>
<a href="https://ihsm-bionas.uma.es/jbrowse/?data=data/carambola" class="btn btn-info sps-btn" role="button">Genome Browser</a>
<a href="/easy_gdb/tools/blast/blast_input.php" class="btn btn-info sps-btn" role="button">BLAST</a>
<a href="/easy_gdb/tools/fasta_download.php" class="btn btn-info sps-btn" role="button">Sequence Extraction</a>
<a href="/easy_gdb/tools/annot_input_list.php" class="btn btn-info sps-btn" role="button">Annotation Extraction</a>
<a href="https://ihsm-bionas.uma.es/easy_gdb/downloads.php" class="btn btn-info sps-btn" role="button">Downloads</a>
<br style="clear:both"/>

<p>
  <em>Averrhoa carambola</em> (carambola or starfruit) is a tropical fruit species native to Southeast Asia and belongs to the family Oxalidaceae. It is valued for its distinctive star-shaped cross-section, sweet-tart flavor, and high juice content, making it a popular fruit in tropical regions and beyond.
</p>

<ul>
  <li>
    <strong>Floral and Fruit Characteristics:</strong>  
    The fruit develops through distinct color indices that indicate its ripeness: green (unripe), yellow (ripe), and golden yellow (overripe). Carambola has a juicy, succulent flesh with a sweet and slightly acidic flavor, making it suitable for fresh consumption or juice production.
  </li>

  <li>
    <strong>Geographic Production:</strong>  
    Malaysia is the largest exporter of carambola globally, exporting over 90% of its production primarily to Europe. Other significant producers include India, the Philippines, and other Southeast Asian countries. Malaysia's annual production is valued at 20–30 million MYR, with exports steadily increasing since 1989.
  </li>

  <li>
    <strong>Phytochemical and Antioxidant Properties:</strong>  
    Carambola contains alkaloids, flavonoids, tannins, and high levels of polyphenolic antioxidants such as L-ascorbic acid, epicatechin, and gallic acid. These compounds contribute to its health benefits, including antioxidant activity. Notably, the byproducts of juice extraction (pomace) have higher antioxidant activity than the juice itself, offering potential for functional food applications.
  </li>

  <li>
    <strong>Traditional and Medicinal Uses:</strong>  
    Starfruit has been used in traditional medicine for treating various ailments. Its rich phytochemical profile supports its role in promoting health and as an ingredient in functional products.
  </li>

  <li>
    <strong>Economic Importance:</strong>  
    Carambola is a significant commodity in tropical fruit markets. Its export value grew by 2.3% annually from 1992 to 2002, with Malaysia ranking third in fruit export value during this period. The increasing global demand for tropical juices has further boosted its economic significance.
  </li>
</ul>
<br>

<h3>Genome assemblies</h3>
<p>
  There are several sequenced genomes available of starfruit:
</p>
<ul>
  <li>assembly by <a href=https://doi.org/10.1038/s41438-020-0307-3>Wu et al., 2020</a>. Available at the <a href="https://ngdc.cncb.ac.cn/gwh/Assembly/946/show" target="_blank">Genome Warehouse</a>.</li>
  <li>assembly by <a href=https://doi.org/10.1038/s41438-020-0306-4>Fan et al., 2020</a>. Available at the <a href="https://db.cngb.org/data_resources/assembly/CNA0002506" target="_blank">China National GeneBank database.</a> </li>
</ul>
<br>

<h3>Genome assembly stats</h3>
<p>
  Overall statistics on <em>A. carambola</em> genome assemblies published by <a href="https://doi.org/10.1038/s41438-020-0307-3" target="_blank">Wu et al. 2020</a> (left) and by <a href="https://doi.org/10.1038/s41438-020-0306-4" target="_blank">Fan et al. 2020</a> (right).
</p>
<?php include_once("carambola_genome_table.php");?>
* Reference genome for IHSM Subtropicals
<br>
<sup>a</sup> No genome available
<br>
<sup>b</sup> BUSCO based on embryophyta_odb12 dataset
<br><br>

<h3>Taxonomy</h3>
<?php include_once("carambola_classification_table.php");?>
<br>
